...
$ch1=mysql_query("select * from cultural where name='$nam'");

$ch2=mysql_query("select * from tech where name='$nam'");
if((mysql_num_rows($ch1)>0)||(mysql_num_rows($ch2)>0))
{



$sel1=mysql_query("select * from cultural where name='$nam'");
while($row=mysql_fetch_array($sel1))
	{
		$msg1=$row['description'];
	}
	
$sel2=mysql_query("select * from tech where name='$nam'");
while($row=mysql_fetch_array($sel2))
	...
	if($msg1===NULL)
	{
		$flg=2;
		$msg=$msg2;
		$sel=mysql_query("select * from tech where name='$nam'");
	...
	else
	{
		$flg=1;
		$msg=$msg1;
		$sel=mysql_query("select * from cultural where name='$nam'");
		while($row=mysql_fetch_array($sel))
	{
		$im=$row['image'];
	}
  ...
  <div class="container">
	...
	<?php
		if($flg==1)
			{
	?>
      <div class="col-md-6"> <img src="admin/action/cultural/<?php echo htmlspecialchars($im);?>" class="img-responsive"> </div>
	<?php
		}
		if($flg==2)
			{
	?>
		 <div class="col-md-6"> <img src="admin/action/tech/<?php echo htmlspecialchars($im);?>" class="img-responsive"> </div>
	<?php
		}
	?>
...
<div id="contact-section" class="text-center">
  ...
  <div class="section-title center">
	...
	<embed height="500" width="100%" src="register.php?pag=<?php echo htmlspecialchars($nam);?>">	
